...
<nav id="navbar">
                ...
                <div class="nav-user-menu" x-data="{ open: false }" @keydown.escape.window="open = false">
                    ...
                    >
                        @if (auth()->user()->avatar_url)
                            <img src="{{ auth()->user()->avatar_url }}" alt="{{ auth()->user()->name }}" class="nav-user-avatar">
                        @else
                            <span class="nav-user-initial">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                        @endif
                    </button>
    ...
    {{--
        MOBILE HAMBURGER — panel personal (diary, history, dll.)
        Hanya tampil di mobile. Di desktop hamburger disembunyikan via CSS.
    --}}
    ...
    {{-- Mobile slide-out panel — personal menu --}}
    <div class="nav-mobile-panel" data-menu-panel>
        @auth
            {{-- Avatar + nama di paling atas --}}
            <div class="nav-mobile-identity">
                ...
                @endif
                    <div class="nav-mobile-identity-avatar">
                        @if (auth()->user()->avatar_url)
                            <img src="{{ auth()->user()->avatar_url }}" alt="{{ auth()->user()->name }}" class="nav-mobile-avatar">
                        @else
                            <span class="nav-mobile-initial">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                        @endif
                    </div>
            ...
            {{-- Logout — di luar section, di bawah dengan separator --}}
